...
from ..backends.transformers_qwen25_vl_classifier_backend import (
    TransformersQwen25VLClassifierBackend,
)
...
def parse_qwen_binary_output(raw: str) -> tuple[str, list[str], str | None]:
    text = (raw or "").strip()
    if not text:
...
class Qwen25VLBinarySafetyClassifier(GuardrailModel):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.backend = TransformersQwen25VLClassifierBackend(
    ...
    def classify_batch(self, samples: list[Sample]) -> list[Verdict]:
        ...
        try:
            ...
            for sample in samples:
                ...
                label, cats, error_reason = parse_qwen_binary_output(raw)
                verdicts.append(
                    Verdict(
                        label=label,
                        categories=cats,
